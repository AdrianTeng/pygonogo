...
class Display:
    ...
    def setup_images(self):
        ...
        for targ in range(self.geom['numtargs']):
            self.notargs.append(TextStim(self.win, text='0', pos=self.geom['target_centers'][targ], height=.3))
            self.gotargs.append(TextStim(self.win, text='1', pos=self.geom['target_centers'][targ], height=.3))
            self.gotargs.append(TextStim(self.win, text='2', pos=self.geom['target_centers'][targ], height=.3))
    ...
    def display_message(self, message, height=None):
        box = TextStim(self.win, text=message, height=height)
        box.draw()
        self.win.flip()
